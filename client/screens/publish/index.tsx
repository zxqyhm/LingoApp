...
export default function PublishScreen() {
  ...
  const pickVideo = async () => {
    try {
      ...
      if (!result.canceled && result.assets) {
        ...
        setSelectedImages([...selectedImages, videoUri]);
      }
    } catch (error) {
  ...
  const uploadImages = async (): Promise<string[]> => {
    const uploadPromises = selectedImages.map(async (uri) => {
      ...
      const response = await fetch(
        `${env.backendBaseUrl}/api/v1/upload/image`,
        {
          method: 'POST',
          body: formData,
        }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // 发布帖子
      const response = await fetch(
        `${env.backendBaseUrl}/api/v1/posts`,
        {
          method: 'POST',
            ...
            {selectedImages.length === 0 && !isUploading && (
              <View style={styles.mediaButtonsContainer}>
                ...
                <TouchableOpacity style={styles.mediaButton} onPress={pickVideo}>
                  <FontAwesome6 name="video" size={24} color={theme.textMuted} />
                  <ThemedText
                    variant="caption"
                    color={theme.textMuted}
                    style={{ marginTop: 8 }}
